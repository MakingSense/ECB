module.exports = function (grunt) {
  ...
  var config = {
    ...
    folder_dev: '../', // If this path gets changed, remember to update .gitignore with the proper path to ignore images and css
    folder_build: 'build',
    folder_dist: 'dist',
  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      scss: {
        files: ['<%= config.folder_assets %>/**/*.scss'],
        tasks: [
          'sass:ui',
          'postcss',
          // 'sassdoc'
      ...
      js: {
        files: '<%= config.folder_assets %>/templates/**/*.js',
        tasks: ['uglify:dev']
      },
      templates: {
        files: [
          '<%= config.folder_assets %>/templates/**/*.php'
        ],
        tasks: ['processhtml']
      },
      images: {
    ...
    bowercopy: {
      ...
      dev: {
        files: {
          ...
          '<%= config.folder_dev %>/js/vendor/owl.carousel.min.js': 'owl.carousel/dist/owl.carousel.min.js'
        }
      }
    },
    ...
    uglify: {
      ...
      dev: {
        ...
        files: [{
          '<%= config.folder_dev %>/js/scripts.min.js': ['<%= config.folder_assets %>/templates/**/*.js']
        }]
      },
      deploy: {
        files: [{
